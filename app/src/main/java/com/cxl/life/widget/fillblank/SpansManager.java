package com.cxl.life.widget.fillblank;
...
public class SpansManager {
        ...
        Spanned spanned = Html.fromHtml(quesOptionAsksResult, null, new Html.TagHandler() {
            ...
            public void handleTag(boolean opening, String tag, Editable output, XMLReader xmlReader) {
                if (tag.equalsIgnoreCase(FILL_TAG_NAME) && opening) {
                    ...
                    paint.setColor(mTv.getResources().getColor(R.color.colorPrimaryDark));
    ...
    public void setSpanChecked(int position){
        ...
        for (int i = 0 ; i < mSpans.size();i++) {
            ...
            if (i == position) {
                replaceSpan.setDrawTextColor(R.color.colorAccent);
            }else {
                replaceSpan.setDrawTextColor(R.color.colorPrimaryDark);
            }
            mTv.invalidate();
        }
    }
...
//                    Selection.removeSelection(buffer);
    ...
    //获取每一条的文本
    public String getItemAnswer(int position){
        if (mCheckedSpan != null) {
            mCheckedSpan.mText = mEt.getText().toString();
        }
        if(position>-1&&mSpans.size()>position){
            return mSpans.get(position).mText;
        }else{
            return "";
        }
    }
